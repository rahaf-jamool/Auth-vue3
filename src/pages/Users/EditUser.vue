<template>
  <div>
    <div class="create">
      ...
      <div class="child_4">
        <button @click="updateUser()" class="btn" id="btnAdd">Update</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      axios
        .get(`/api/auth/user/getById/${this.$route.params.id}`)
        .then((res) => {
          console.log(res);
          this.users.fullName = res.data.User.fullName;
          this.users.password = res.data.User.password;
          this.users.email = res.data.User.email;
          this.users.userType = res.data.User.userType;
          this.users.role_id = res.data.User.role_id;
        })
        .catch(function (error) {
          if (error) {
            console.log("error:", error);
            alert(
              `error !! Sorry User by id request had error we can not return old data.. work soon`
            );
          }
        });
    },
    updateUser() {
      ...
      if (this.users.fullName == "") {
        ...
        axios
          .put(`/api/auth/user/update/${this.$route.params.id}`, this.users)
          .then(function (response) {
            console.log(response.data);
            document.getElementById("sp").classList.toggle("cvs");
            self.Massage_success = "Update User Request Success";
