<template>
  <div>
    <div class="create">
      ...
      <div class="child_4">
        <button @click="updateRole()" class="btn" id="btnAdd">Update</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      axios
        .get(`/api/auth/role/getById/${this.$route.params.id}`)
        .then((res) => {
    ...
    updateRole() {
      ...
      if (this.roles.name == "") {
        ...
        axios
          .put(`/api/auth/role/update/${this.$route.params.id}`, this.roles)
          .then(function (response) {
            console.log(response.data);
            document.getElementById("sp").classList.toggle("cvs");
            self.Massage_success = "Update Role Request Success";
            // self.Massage_success = response.data.content;
            document.getElementById("su").classList.toggle("cvs");
            setTimeout(() => {
              self.$router.push({ name: "rules" });
            }, 2000);
          })
          .catch(function (error) {
