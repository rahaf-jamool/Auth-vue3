<template>
  <div>
    <div class="create">
      ...
      <div class="child_4">
        <button @click="updatePost()" class="btn" id="btnAdd">Update</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    fetch() {
      axios
        .get(`/api/post/getById/${this.$route.params.id}`)
        .then((res) => {
          console.warn("post", res);
          this.posts.title = res.data.Post.title;
          this.posts.body = res.data.Post.body;
          this.posts.user_id = res.data.Post.user_id;
        })
        .catch(function (error) {
          if (error) {
            console.log("error:", error);
            alert(
              `error !! Sorry Post by id request had error we can not return old data.. work soon`
            );
          }
        });
    },
    updatePost() {
      ...
      if (this.posts.title == "") {
        this.Massage_warning =
          "Please enter the title field in Arabic because it is required";
        document.getElementById(`m`).classList.toggle("cvs");
      } else if (this.posts.body == "") {
        this.Massage_warning =
          "Please select the description because it is required";
        document.getElementById(`m`).classList.toggle("cvs");
      } else if (this.posts.user_id == "") {
        this.Massage_warning = "Please select the user because it is required";
        document.getElementById(`m`).classList.toggle("cvs");
      } else {
        document.getElementById("sp").classList.toggle("cvs");
        axios
          .put(`/api/auth/post/update/${this.$route.params.id}`, this.posts)
          .then(function (response) {
            console.log(response.data);
            document.getElementById("sp").classList.toggle("cvs");
            self.Massage_success = "Update Post Request Success";
